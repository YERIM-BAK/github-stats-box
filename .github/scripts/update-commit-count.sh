...
# Accept GH_TOKEN or GH_PAT environment variable (GH_TOKEN preferred)
if [ -n "${GH_TOKEN:-}" ]; then
  AUTH_TOKEN="$GH_TOKEN"
elif [ -n "${GH_PAT:-}" ]; then
  AUTH_TOKEN="$GH_PAT"
else
  echo "GH_TOKEN or GH_PAT is not set. Exiting. Please add the secret GH_TOKEN (or GH_PAT)."
  exit 0
fi
...
while :; do
  ...
  # If API returned an error object, stop
